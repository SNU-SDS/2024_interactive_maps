...
```{r}
df_2022 <- read_excel("D:/My R/Korea Census Data Manipulation/9 Migration/DM_Comp_Code_Mig_2022_Sidogungu.xlsx") 
...
df <- df_2022 |> 
  ...
  filter(
    (Ori_CD > 10000 & Ori_CD < 20000) & (Des_CD > 10000 & Des_CD < 20000) & (Ori_CD != Des_CD)
  )
```
...
```{r}
#| results: hide
seoul <- st_read("D:/My R/Korean Administrative Areas/행정구역 셰이프 파일/2 Original Cleaning/2022_4Q/SEOUL_SIDO_2022_4Q.shp", options = "ENCODING=CP949")
seoul_gu <- st_read("D:/My R/Korean Administrative Areas/행정구역 셰이프 파일/2 Original Cleaning/2022_4Q/SEOUL_GU_2022_4Q.shp", options = "ENCODING=CP949")
```
...
플로 데이터를 방향적 지역쌍별 플로량을 포함하고 있는 데이터이다.
